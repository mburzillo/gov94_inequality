...
hp_mayors_clean <- hp_mayors %>%
  ...
  mutate(tot_bmayor = sum(bmayor_to_sum, na.rm = T)) %>%
  ungroup() %>%
  mutate(bever = ifelse(tot_bmayor > 0, 1, 0), status = "match", hp = 1) %>%
  select(city, stateab, year, bmayor, status, hp)
...
full_join(vogl_mayor_clean, hp_mayors_clean, by = c("cityname" = "city", "state" = "stateab", "year" = "year", "bmayor" = "bmayor"))

# previous stats:
# vogl clean 14214
# hp clean 1088
# total: 15302
# original join has 14707
